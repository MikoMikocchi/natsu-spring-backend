...
/**
 * Two in-process Caffeine caches (no Redis -- a single-instance deployment has no need for a
 * shared cache). Invalidation is by cache key, not eviction: {@code User.dictCacheVersion} is
 * part of both keys and gets bumped on toggle, so old entries just age out by TTL instead of
 * needing coordinated {@code @CacheEvict} calls.
 */
@Configuration
@EnableCaching
