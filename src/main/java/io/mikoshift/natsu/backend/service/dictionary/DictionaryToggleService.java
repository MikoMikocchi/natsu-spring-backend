...
public class DictionaryToggleService {
    ...
    /**
     * Creates or deletes the opt-out row, then bumps the user's cache-version counter. {@code
     * user} arrives detached (loaded in an earlier request-handling transaction by the auth
     * filter), so the version bump needs an explicit save -- merely mutating a detached entity's
     * field does nothing without one.
     */
    @Transactional
    public void toggle(User user, UUID dictionaryId) {
        Dictionary dictionary =
                dictionaryRepository.findById(dictionaryId).orElseThrow(() -> new NotFoundException("Dictionary not found"));
