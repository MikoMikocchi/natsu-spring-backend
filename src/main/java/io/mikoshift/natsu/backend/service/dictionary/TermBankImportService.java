 ...
 * the server-side catalog. This is a seed-time operation (see {@code DictionarySeedRunner}), not a
 * public upload endpoint -- dictionary catalog content is server-curated.
 */
@Service
...
public class TermBankImportService {
  ...
  public Dictionary importZip(String catalogId, byte[] zipBytes) {
    ...
    if (total == 0) {
      throw new TermBankImportException(
          "Dictionary archive has no term_bank_*.json files with entries");
    }
