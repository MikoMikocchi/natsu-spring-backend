 ...
 * Covers a standard, well-formed EPUB 2/3 package; it does not handle DRM/encryption or fall back
 * to a table-of-contents-derived reading order when the spine is missing.
 */
@Component
public class EpubImporter implements BookImporter {
  ...
  public ImportedBook importFrom(byte[] sourceBytes) {
    ...
    Document opf =
        Jsoup.parse(new String(opfBytes, StandardCharsets.UTF_8), "", Parser.xmlParser());
  ...
  private static String parseOpfPath(byte[] containerXml) {
    Document container =
        Jsoup.parse(new String(containerXml, StandardCharsets.UTF_8), "", Parser.xmlParser());
