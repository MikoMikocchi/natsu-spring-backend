...
class BookImportPersistence {
     ...
     * executor finishing the real import a moment later) from being clobbered by a recovery pass
     * that read stale data, and avoids the self-invocation trap of calling a {@code @Transactional}
     * method from another method on this same bean.
     *
     ...
     * loop. It's still tracked and reported back so the caller can log distinctly once a document
     * has been (re)flagged more times than expected, which would itself be a signal something is
     * wrong (e.g. this method being called repeatedly for a document that should have already left
     * PENDING).
     *
