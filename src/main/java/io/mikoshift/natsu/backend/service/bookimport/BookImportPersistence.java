...
class BookImportPersistence {
  ...
  @Transactional
  void applySuccess(
      UUID documentId, String title, int charCount, String searchText, StoredPackage stored) {
    documentRepository
        .findById(documentId)
        .ifPresent(
            document -> {
  ...
  RecoveryOutcome recoverStaleDocument(UUID documentId, int maxAttempts) {
    ...
    document.setStatus(Document.Status.FAILED);
    document.setImportError(
        "Import was interrupted and could not be resumed; please re-upload the file.");
    document.setUpdatedAtMs(Instant.now().toEpochMilli());

    return attempts > maxAttempts
        ? RecoveryOutcome.FAILED_ATTEMPTS_EXCEEDED
        : RecoveryOutcome.FAILED_STALE;
  }
