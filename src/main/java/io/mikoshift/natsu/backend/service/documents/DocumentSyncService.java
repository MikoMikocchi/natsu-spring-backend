...
public class DocumentSyncService {
    ...
    /**
     * Batch-upserts up to 100 documents. Each row is merged independently: a row whose
     * updatedAtMs is older than what's stored is a no-op (the stored state is returned as-is),
     * exactly like the single-row settings merge, just applied per item in one request. A row
     * that doesn't exist yet for this user is always created (there is no prior state to compare
     * against) -- this is how a device pushes a locally-created document for the first time.
     */
    @Transactional
