...
public class TokenService {
    ...
    /**
     * Rotates the access/refresh pair for the session identified by {@code presentedRefreshToken}.
     * If the presented token is the *previous* refresh token (a client retrying a request that
     * raced a prior rotation), the already-rotated current pair is returned instead of rotating
     * again, avoiding invalidating the client's now-current session.
     */
    @Transactional
    ...
    public void revokeAll(User user) {
        Instant now = Instant.now();
        authTokenRepository.findAllByUserAndRevokedAtIsNullOrderByCreatedAtDesc(user)
