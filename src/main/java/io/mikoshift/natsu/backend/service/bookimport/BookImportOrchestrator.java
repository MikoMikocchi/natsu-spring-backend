...
/**
 * Runs off the request thread on {@code bookImportExecutor}. A malformed source file
 * ({@link ImportException}) is a permanent failure handled inline, with no retry. A storage I/O
 * hiccup ({@link TransientImportException}) is retried up to 3 times with exponential backoff;
 * once retries are exhausted, {@link #recover} marks the document failed the same way a permanent
 * failure would. The actual file parsing/zip-building work runs outside any DB transaction --
 * only the short read-before and write-after steps (in {@link BookImportPersistence}) are
 * transactional, so a slow import never holds a pooled connection open.
 */
@Component
...
public class BookImportOrchestrator {
    ...
    @Retryable(retryFor = TransientImportException.class, maxAttempts = 3, backoff = @Backoff(delay = 1000, multiplier = 2))
