...
public interface DocumentRepository extends JpaRepository<Document, UUID> {
   ...
   */
  @Query(
      "select d from Document d where d.user = :user and d.deletedAt is null "
  ...
  @Query(
      "select coalesce(sum(d.packageSizeBytes), 0) from Document d where d.user = :user and d.deletedAt is null")
