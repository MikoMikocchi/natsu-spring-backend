 ...
 * itself, so controllers that need to know "which device session is this request" (logout,
 * session list "current" flag) can read it off {@code Authentication#getCredentials()}.
 */
@Component
...
public class BearerTokenAuthenticationFilter extends OncePerRequestFilter {
    ...
    private void authenticate(AuthToken token) {
        var authentication =
                new UsernamePasswordAuthenticationToken(token.getUser(), token, List.of());
