...
class BookImportIntegrationTest {
    ...
    void importingPlainTextEndsUpReadyWithADownloadablePackage() throws Exception {
        ...
        MockMultipartFile file = new MockMultipartFile(
                "file", "diary.txt", "text/plain", "Dear diary,\n\nToday was good.".getBytes(StandardCharsets.UTF_8));

        String importResponse = mockMvc.perform(
        ...
        byte[] packageBytes = mockMvc.perform(
                        get("/v1/documents/" + documentId + "/package").header("Authorization", "Bearer " + token))
                .andExpect(status().isOk())
                .andReturn()
                .getResponse()
    ...
    void malformedEpubEndsUpFailedWithAnErrorMessage() throws Exception {
        ...
        MockMultipartFile file = new MockMultipartFile(
                "file", "broken.epub", "application/epub+zip", "not actually a zip".getBytes(StandardCharsets.UTF_8));
    ...
    void directPackageUploadValidatesTheZipAndUpdatesTheDocument() throws Exception {
        ...
        MockMultipartFile invalidZip = new MockMultipartFile(
                "package", "package.zip", "application/zip", "not a zip".getBytes(StandardCharsets.UTF_8));
