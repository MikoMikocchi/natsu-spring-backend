...
class JapaneseDeinflectorTest {
    ...
    private List<String> candidateWords(String surface) {
        return deinflector.deinflect(surface).stream().map(Deinflection::candidate).toList();
    }
    ...
    void deinflectionCarriesRuleNameAndDescription() {
        ...
        Deinflection pastTense = results.stream().filter(d -> d.candidate().equals("食べる")).findFirst().orElseThrow();
