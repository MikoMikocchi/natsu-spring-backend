 ...
 * save touch that column post-insert -- {@code updatable = false} -- so a raw SQL update is used
 * to simulate "this document has been sitting here since before the app last restarted"), then
 * invokes {@link StaleImportRecoveryService#recoverStaleImports()} directly rather than waiting on
 * the real schedule.
 */
