...
class DictionaryIntegrationTest {
  ...
  @Autowired private TermBankImportService termBankImportService;
  ...
  void importListLookupToggleAndCacheInvalidationFullFlow() throws Exception {
    ...
    Dictionary dictionary =
        termBankImportService.importZip("test-catalog-" + System.nanoTime(), buildFixtureZip());
    ...
    mockMvc
        .perform(get("/v1/dictionaries").header("Authorization", "Bearer " + token))
        .andExpect(status().isOk())
        .andExpect(
            jsonPath("$.dictionaries[?(@.id == '" + dictionaryId + "')].enabled")
                .value(org.hamcrest.Matchers.contains(true)))
        .andExpect(
            jsonPath("$.dictionaries[?(@.id == '" + dictionaryId + "')].term_count")
    ...
    mockMvc
        .perform(
            patch("/v1/dictionaries/" + dictionaryId + "/toggle")
                .header("Authorization", "Bearer " + token))
  ...
  void lookupRanksDirectMatchesAboveDeinflectedOnes() throws Exception {
    ...
    termBankImportService.importZip(
        "rank-catalog-" + System.nanoTime(), buildAmbiguousFixtureZip());
