...
class AuthFlowIntegrationTest {
  ...
  void registerLoginUseTokenListSessionsRevokeRefreshLogout() throws Exception {
    ...
    mockMvc
        .perform(get("/v1/auth/user").header("Authorization", "Bearer " + firstToken))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.user.email").value(email));

    // Missing/invalid tokens are rejected with the API's standard error shape.
    mockMvc
        .perform(get("/v1/auth/user"))
        .andExpect(status().isUnauthorized())
        .andExpect(jsonPath("$.errors.base").exists());
    mockMvc
        .perform(get("/v1/auth/user").header("Authorization", "Bearer garbage"))
        .andExpect(status().isUnauthorized());

    // Two device sessions are listed, each correctly flagged as "current" relative to its own
    // token.
    String sessionsAsDeviceA =
        mockMvc
            .perform(get("/v1/auth/sessions").header("Authorization", "Bearer " + firstToken))
            .andExpect(status().isOk())
            .andExpect(jsonPath("$.length()").value(2))
            .andReturn()
            .getResponse()
            .getContentAsString();
    List<Integer> deviceBSessionIds =
        JsonPath.read(sessionsAsDeviceA, "$[?(@.name == 'device-b')].id");
    ...
    mockMvc
        .perform(
            delete("/v1/auth/sessions/" + deviceBSessionId)
                .header("Authorization", "Bearer " + firstToken))
        .andExpect(status().isNoContent());
    mockMvc
        .perform(get("/v1/auth/user").header("Authorization", "Bearer " + secondToken))
        .andExpect(status().isUnauthorized());
    ...
    mockMvc
        .perform(get("/v1/auth/user").header("Authorization", "Bearer " + rotatedToken))
        .andExpect(status().isOk());

    // Logout revokes the session tied to the presented token.
    mockMvc
        .perform(post("/v1/auth/logout").header("Authorization", "Bearer " + rotatedToken))
        .andExpect(status().isNoContent());
    mockMvc
        .perform(get("/v1/auth/user").header("Authorization", "Bearer " + rotatedToken))
        .andExpect(status().isUnauthorized());
  ...
  void deleteAccountRequiresCorrectPassword() throws Exception {
    ...
    mockMvc
        .perform(get("/v1/auth/user").header("Authorization", "Bearer " + token))
        .andExpect(status().isUnauthorized());
  }
