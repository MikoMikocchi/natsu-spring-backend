...
class DocumentIntegrationTest {
  ...
  private String syncDoc(String token, String id, String title, long updatedAtMs, boolean deleted)
      throws Exception {
    return mockMvc
        .perform(
  ...
  void syncCreatesUpdatesAndSoftDeletesWithConflictResolution() throws Exception {
    ...
    assertThat((Integer) JsonPath.read(created, "$.documents[0].updated_at_ms")).isEqualTo(1000);
    ...
    mockMvc
        .perform(get("/v1/documents/" + id).header("Authorization", "Bearer " + token))
  ...
  void unknownDocumentReturns404WithErrorShape() throws Exception {
    ...
    mockMvc
        .perform(
            get("/v1/documents/" + UUID.randomUUID()).header("Authorization", "Bearer " + token))
  ...
  void documentsAreScopedPerUser() throws Exception {
    ...
    mockMvc
        .perform(get("/v1/documents/" + id).header("Authorization", "Bearer " + tokenB))
        .andExpect(status().isNotFound());
  }
  ...
  void searchMatchesTitleAndReturnsSnippet() throws Exception {
    ...
    mockMvc
        ...
        .andExpect(
            jsonPath("$.results[0].matches[0].snippet")
