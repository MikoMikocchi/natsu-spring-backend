...
class DocumentIntegrationTest {
    ...
    void searchMatchesTitleAndReturnsSnippet() throws Exception {
        ...
        mockMvc.perform(get("/v1/documents/search").header("Authorization", "Bearer " + token).param("q", "gatsby"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.results.length()").value(1))
                .andExpect(jsonPath("$.results[0].title").value("The Great Gatsby"))
                .andExpect(jsonPath("$.results[0].matches[0].snippet").value(org.hamcrest.Matchers.containsStringIgnoringCase("gatsby")));
    }
