...
class ReaderSettingIntegrationTest {
  ...
  @Test
  void firstReadCreatesDefaultsAndSubsequentPatchesAreConflictResolvedByTimestamp()
      throws Exception {
    String token = registerAndGetToken("settings@example.com");

    // No row exists yet; GET creates and returns the defaults.
    mockMvc
        .perform(get("/v1/settings/reader").header("Authorization", "Bearer " + token))
  ...
  void settingsAreScopedPerUser() throws Exception {
    ...
    mockMvc
        .perform(get("/v1/settings/reader").header("Authorization", "Bearer " + tokenB))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.settings.theme").value("LIGHT"));
  }
}
