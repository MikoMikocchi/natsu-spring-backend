...
class BookImportIntegrationTest {
  ...
  void importingPlainTextEndsUpReadyWithADownloadablePackage() throws Exception {
    ...
    mockMvc
        .perform(
            request(HttpMethod.HEAD, "/v1/documents/" + documentId + "/package")
                .header("Authorization", "Bearer " + token))
  ...
  void epubTitleFromMetadataOverridesTheFilenameFallback() throws Exception {
    ...
    MockMultipartFile file =
        new MockMultipartFile("file", "source.epub", "application/epub+zip", epub);
    ...
    mockMvc
        .perform(get("/v1/documents/" + documentId).header("Authorization", "Bearer " + token))
  ...
  void importRejectsUnsupportedFileExtensions() throws Exception {
    ...
    mockMvc
        .perform(
            multipart("/v1/documents/import").file(file).header("Authorization", "Bearer " + token))
  ...
  void directPackageUploadValidatesTheZipAndUpdatesTheDocument() throws Exception {
    ...
    mockMvc
        .perform(
            multipart(HttpMethod.PUT, "/v1/documents/" + documentId + "/package")
                .file(invalidZip)
                .header("Authorization", "Bearer " + token))
        .andExpect(status().isUnprocessableEntity());

    byte[] validZip = buildZip(Map.of("manifest.json", "{\"version\":1,\"sections\":[]}"));
    MockMultipartFile validPackage =
        new MockMultipartFile("package", "package.zip", "application/zip", validZip);
    mockMvc
        .perform(
            multipart(HttpMethod.PUT, "/v1/documents/" + documentId + "/package")
                .file(validPackage)
